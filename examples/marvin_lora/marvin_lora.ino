...
String  set_nwkskey = "ab8ba06e6e6c3f89a6020436de88fe07";
String  set_appskey = "084c5427d0654af3d60e09dd47bd38fc";
String  set_devaddr = "142042FD";
...
void printHex(unsigned char hex)
{
  ...
  Serial1.print(hexTable[hex /16]);
  Serial1.print(hexTable[hex % 16]);
 // Serial1.print(' ');
}

void send_LoRa_data_i(int tx_port, String rawdata)
{
    Serial.print("Now sending: mac tx uncnf " + String(tx_port) + String(" "));
    Serial1.print("mac tx uncnf " + String(tx_port) + String(" "));
    for(int i =0; i < rawdata.length(); i++){
        printHex(rawdata.c_str()[i]);
    }
    Serial1.println();
    Serial.println();
    delay(500);
...
void send_LoRa_data(int tx_port, int value)
{
    SenMLFloatRecord rec("temp", SENML_UNIT_DEGREES_CELSIUS, value);
    doc.add(&rec);             

    char buffer[150];    
    memset(buffer, 0, sizeof(buffer));        
    doc.toJson(buffer, sizeof(buffer));
    send_LoRa_data_i(tx_port, buffer);
